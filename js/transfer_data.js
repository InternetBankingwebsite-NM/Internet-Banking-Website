var mybal;
var call_fun = setInterval(function () {
  getbal();
}, 1000);
var amount;
function getbal() {
...
function get_sender() {
  ...
  if (
    ...
    document.getElementById("show_err").innerHTML =
      "Account field is empty or doesn't contain NUMBERS";
  ...
  for (i = 2; i <= used_rows; i++) {
    if (accno == parent.excel_sheet.Cells(i, 5).value) {
      document.getElementById("show_err").innerHTML = "";
      name =
        parent.excel_sheet.Cells(i, 1) +
        " " +
        parent.excel_sheet.Cells(i, 2).value +
        " " +
        parent.excel_sheet.Cells(i, 4);
...
function submit_form() {
  ...
  if (flag == 0) {
    ...
    document.getElementById("show_err").innerHTML =
      "Amount can't contain alphabets.";
    return false;
  } else if (amount <= 0) {
    document.getElementById("show_err").innerHTML =
      "Amount can't be lesser than 1";
    return false;
  ...
  try {
    ...
    parent.user_trans_sheet.Cells(++trans_no_sender, 1).value =
      "Me(Accno=" + parent.excel_sheet.Cells(parent.row_no, 5).value + ")";
    parent.user_trans_sheet.Cells(trans_no_sender, 2).value =
      name + "(Accno=" + accno + ")";
...
function add_to_receiver() {
  ...
  if (parent.getCookie("acc_name") != "") {
    parent.receiver_trans_sheet.Cells(++receiver_trans_no, 1).value =
      parent.getCookie("acc_name") +
      "(Accno=" +
      parent.excel_sheet.Cells(parent.row_no, 5).value +
      ")";
  } else {
    parent.receiver_trans_sheet.Cells(++receiver_trans_no, 1).value =
      parent.getCookie("name_user") +
      "(Accno=" +
      parent.excel_sheet.Cells(parent.row_no, 5).value +
      ")";
  }
  parent.receiver_trans_sheet.Cells(receiver_trans_no, 2).value =
    "Me(Accno=" + accno + ")";
