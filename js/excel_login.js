...
function excel_getdata(user, pass) {
  for (i = 2; i <= used_rows; i++) {
    if (
      user == excel_sheet.Cells(i, 6).value &&
      pass == excel_sheet.Cells(i, 7).value
    ) {
      setCookie(
        "name_user",
        excel_sheet.Cells(i, 1) +
          excel_sheet.Cells(i, 2).value +
...
function get_usersheet() {
  user_trans = excel.Workbooks.Open(
    base_path + excel_sheet.Cells(row_no, 5).value + ".xls"
  );
